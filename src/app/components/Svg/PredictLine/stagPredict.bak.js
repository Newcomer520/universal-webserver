...
export default class PredictLine extends Component {
	render(){
		...
		// 只有第一段先spring長完
		// 長完後才換第二個長
		const config ={ stiffness:200, damping: 23}
		const { points, xOffset, yOffset } = this.props
		return (
			<g>
			<StaggeredMotion
				defaultStyles={points}
				styles={prevInterpolatedStyles => prevInterpolatedStyles.map((whatever, i) => {
					if(i === 0){
						...
						<g transform={`translate(${xOffset}, ${yOffset})`}>
							{
								interpolatingStyles.map((style, i) => {
										if(i%2!==0){
											...
											return (
												<g key={`g${i}`} >
													<line key={`line${i}`} x1={points[i].x} y1={points[i].y} x2={style.x} y2={style.y}
																style={{ stroke: '#e61673', strokeWidth: 2 }}/>

													<line key={`predup${i}`} x1={points[i].x} y1={points[i].y + 50} x2={style.x} y2={style.y + 50}
																style={{ stroke: '#e61673', strokeWidth: 2 }}
																strokeDasharray={1, 3}/>
													<line key={`predbot${i}`} x1={points[i].x} y1={points[i].y - 50} x2={style.x} y2={style.y - 50}
																style={{ stroke: '#e61673', strokeWidth: 2 }}
																strokeDasharray={1, 3}/>
													<line key={`wline${i}`} x1={points[i].x} y1={points[i].y} x2={style.x} y2={style.y}
																style={{stroke: '#e61673', strokeWidth: 100, opacity: 0.1}}
																/>
												</g>
											)
										}
